      ...
      <div className="flex gap-4 mb-8">
        ...
        <div className="flex-1">
           <textarea 
             ...
             className="w-full bg-gray-50 border rounded-lg p-3 text-sm h-24 resize-none"
           />
           <div className="flex justify-end mt-2">
              <button 
                onClick={handleSubmit}
                disabled={!isLoggedIn || !inputValue.trim()}
                className="bg-[#d32f2f] text-white px-6 py-1.5 rounded-full text-sm disabled:opacity-50"
              >
                发布
              </button>
           </div>
      ...
      <div className="space-y-6">
        {comments.map(c => (
           <div key={c.id} className="flex gap-4">
              <div className="w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center font-bold text-gray-600 flex-shrink-0 text-xs">{c.user.slice(0,1)}</div>
              <div className="flex-1">
                 <div className="flex items-center justify-between mb-1">
                    <span className="text-sm font-bold text-gray-800">{c.user}</span>
                    <span className="text-xs text-gray-400">{c.time}</span>
                 </div>
                 <p className="text-gray-600 text-sm mb-2">{c.content}</p>
                 <button className="flex items-center gap-1 text-xs text-gray-400 hover:text-[#d32f2f]"><Heart size={14}/> {c.likes}</button>
              </div>
           </div>
        ))}
      </div>
