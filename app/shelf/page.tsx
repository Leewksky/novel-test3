    ...
    <div className="min-h-screen bg-[#f4f5f7]">
      ...
      <div className="container mx-auto px-4 py-8">
              ...
              {shelf.map(book => (
                <div key={book.id} className="flex gap-4 p-3 border rounded-lg hover:shadow-md transition bg-white relative group">
                   <Link href={`/n/${book.id}`} className="w-20 h-28 flex-shrink-0">
                      <img src={book.coverUrl} className="w-full h-full object-cover rounded shadow" />
                   </Link>
        ...
        <div className="bg-white rounded-lg p-6 shadow-sm">
           <h2 className="text-xl font-bold mb-6 flex items-center gap-2 border-l-4 border-gray-400 pl-3">
             <Clock size={20} /> 最近阅读
          </h2>
